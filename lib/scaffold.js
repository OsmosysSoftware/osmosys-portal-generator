...
function patchPackageJson(outputDir, options) {
  ...
  pkg['lint-staged'] = {
    // --no-warn-ignored: don't fail on eslint-ignored generated files
    // (e.g. src/app/core/types/api.types.ts) when they're staged for commit.
...
function installExtraDeps(outputDir) {
  // Pin @angular/animations to the exact Angular version the CLI just installed.
  // A bare `npm install @angular/animations` re-resolves against the fresh tree
  // and can pick an older major (e.g. 20.x), whose peer deps collide with the
  // installed Angular 21 packages and abort the install with ERESOLVE.
  const rootPkg = JSON.parse(readFileSync(`${outputDir}/package.json`, 'utf-8'));
  const ngVersion = rootPkg.dependencies['@angular/core'];
  const ngMajor = ngVersion.match(/\d+/)[0];
  ...
  const devDeps = [
    // angular-eslint's major tracks the Angular major; bare install grabs the
    // newest (e.g. 22.x) and conflicts with the installed Angular CLI 21.
    `angular-eslint@^${ngMajor}.0.0`,
    'eslint',
